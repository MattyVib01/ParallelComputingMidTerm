...
\begin{itemize}
   ...
   \item Assegnazione dei punti ai cluster: l'algoritmo andrà a cercare per ogni punto il centroide a lui più vicino e di conseguenza gli assegnerà il corrispondente cluster.
...
\begin{lstlisting}[caption={Esempio di parallelizzazione del ciclo di assegnamento.}, label={lst:openmp}]
#pragma omp parallel for private(i, j) shared(points, centroids)
for (i = 0; i < num_points; i++) {
    double min_dist = __DBL_MAX__;
    for (j = 0; j < K; j++) {
        double d = euclidean_distance(points[i], centroids[j]);
        if (d < min_dist) {
            min_dist = d;
            points[i].cluster = j;
        }
