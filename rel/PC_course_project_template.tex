...
%%%%%%%%% TITLE
\title{K-Means: Confronto fra la versione sequenziale e le implementazioni parallele sviluppate con OpenMP }
...
Attraverso il framework \textit{OpenMP} è possibile parallelizzare parte del codice, pertanto andremo a valutare quanto l'impatto della parallelizzazione migliori le prestazioni dell'algoritmo.
\begin{figure}[t]
\begin{center}
...
\begin{itemize}
   ...
   \item \lstinline{#pragma omp for} : la direttiva \textit{omp for} dice al compilatore di distribuire il ciclo che segue fra più thread.
   \item \lstinline{#pragma omp critical}: Questa direttiva identifica una sezione critica del codice che deve essere eseguita da un singolo thread alla volta.
\end{itemize}
...
\end{itemize}
A livello teorico quindi l'approccio SoA dovrebbe darci risultati migliori in termini di speedup rispetto alla versione sequenziale, vedremo nella sezione degli Esperimenti se questo sarà confermato.
...
Andiamo innanzitutto a valutare la struttura della classe \lstinline{Point}, che risulta essere la stessa utilizzata nella versione sequenziale
...
%----------------------------------------------------------
\section{Conclusioni}
Il progetto ha quindi analizzato l’efficienza dell’algoritmo K-Means, facendo un confronto fra
...
\begin{itemize}
   \item L'algoritmo ha dimostrato di essere un ottimo candidato per la parallelizzazione. Entrambi gli approcci (AoS e SoA) si sono dimostrati particolarmente efficaci.
   \item All'aumentare del numero di elementi del dataset la parallelizzazione mostra il suo lato migliore. Infatti si ottengono risultati di gran lunga migliori rispetto alla versione sequenziale.
